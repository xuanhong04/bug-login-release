...
export function VpnCheckButton({
  ...
  const handleCheck = React.useCallback(async () => {
    ...
    try {
      ...
      if (checkResult.is_valid) {
        toast.success(t("vpnCheck.toasts.valid", { name: vpnName }));
      } else {
        toast.error(t("vpnCheck.toasts.invalid", { name: vpnName }));
      }
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      toast.error(t("vpnCheck.toasts.failed", { message: errorMessage }));
  ...
  }, [checkingVpnId, setCheckingVpnId, t, vpnId, vpnName]);
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>
        {isCurrentlyChecking ? (
          <p>{t("vpnCheck.tooltip.checking")}</p>
        ) : result?.is_valid ? (
          <div className="space-y-1">
            <p>{t("vpnCheck.tooltip.valid")}</p>
            <p className="text-xs text-primary-foreground/70">
              {t("vpnCheck.tooltip.checkedAt", {
          ...
          <div>
            <p>{t("vpnCheck.tooltip.invalid")}</p>
            <p className="text-xs text-primary-foreground/70">
              {t("vpnCheck.tooltip.checkedAt", {
                time: formatRelativeTime(result.timestamp),
              })}

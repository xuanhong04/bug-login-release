...
/// Setup function to ensure the BugLogin proxy sidecar exists and cleanup stale proxies.
...
async fn test_traffic_tracking() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let traffic_stats_dir = buglogin_lib::app_dirs::cache_dir().join("traffic_stats");
...
async fn test_bypass_rules_http_direct() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  // Start the proxy sidecar with upstream + bypass rules for "127.0.0.1"
...
async fn test_bypass_rules_in_config() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let proxies_dir = buglogin_lib::app_dirs::proxies_dir();

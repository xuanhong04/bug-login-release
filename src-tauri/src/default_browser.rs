...
mod macos {
  ...
  pub fn is_default_browser() -> Result<bool, String> {
    ...
    let bundle_id = "com.buglogin.desktop";
  ...
  pub fn set_as_default_browser() -> Result<(), String> {
    let bundle_id = CFString::new("com.buglogin.desktop");
    let schemes = ["http", "https"];
      ...
      unsafe {
        ...
        if status != 0 {
          let error_msg = match status {
            -54 => format!(
              "Failed to set as default browser for scheme '{scheme}'. The app is not properly registered as a browser. Please:\n1. Build and install the app properly\n2. Manually set BugLogin as default in System Settings > General > Default web browser\n3. Make sure the app is in your Applications folder"
            ),
            _ => format!(
              "Failed to set as default browser for scheme '{scheme}'. Status code: {status}. Please manually set BugLogin as default in System Settings > General > Default web browser."
            )
          };
...
mod windows {
  ...
  const APP_NAME: &str = "BugLogin";
  const PROG_ID: &str = "BugLogin.HTML";
  ...
  fn register_application(exe_path: &str) -> Result<(), String> {
    ...
    app_key
      .set_value(
        ...
        &"BugLogin - Managed Multi-Profile Browser",
      )
  ...
  fn register_prog_id(exe_path: &str) -> Result<(), String> {
    ...
    prog_id_key
      .set_value("", &"BugLogin Document")
      .map_err(|e| format!("Failed to set ProgID default value: {}", e))?;

    prog_id_key
      .set_value("FriendlyTypeName", &"BugLogin Document")
...
mod linux {
  ...
  const APP_DESKTOP_NAME: &str = "buglogin.desktop";
  ...
  pub fn set_as_default_browser() -> Result<(), String> {
    ...
    if !check_desktop_file_exists() {
      return Err(format!(
        "Desktop file '{}' not found in standard locations. Please ensure the application is properly installed. You can manually set BugLogin as the default browser in your system settings.",
        APP_DESKTOP_NAME
      ));
    }
